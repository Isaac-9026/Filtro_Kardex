...
def load_kardex(file_bytes, filename):
    try:
        ...
        for _, row in df_raw.iterrows():
            ...
            sal_cant  = float(sal_cant)  if pd.notna(sal_cant)  else 0.0
            sal_unit  = float(sal_unit)  if pd.notna(sal_unit)  else 0.0
            sal_total = float(sal_total) if pd.notna(sal_total) else 0.0
            registros.append({
                "Codigo":          codigo,
                "Fecha":           fecha,
                "Tipo":            tipo_comp,
...
def detectar_duplicados(frames):
...
def calcular_saldo_final(df, saldos_iniciales):
    ...
    df["_orden_op"] = df["Tipo_Operacion"].apply(lambda x: 0 if "compra" in str(x).lower() else 1)
...
def render_tabla(dff):
    ...
    cols_rename = {
        "Codigo": "Código", "Fecha": "Fecha", "Tipo": "Tipo", "Serie": "Serie",
        "Numero": "Número", "Tipo_Operacion": "Operación",
        "Ent_Cantidad": "Ent. Cant.", "Ent_Costo_Unit": "Ent. C.Unit", "Ent_Costo_Total": "Ent. C.Total",
        "Sal_Cantidad": "Sal. Cant.", "Sal_Costo_Unit": "Sal. C.Unit", "Sal_Costo_Total": "Sal. C.Total",
        "Saldo_Cantidad": "Saldo Cant.", "Saldo_Costo_Unit": "Saldo C.Unit", "Saldo_Costo_Total": "Saldo C.Total",
    }
    fmt = {
    ...
    saldo_neg = display["Saldo_Negativo"].reset_index(drop=True) if "Saldo_Negativo" in display.columns else pd.Series([False] * len(display))
    display = display.drop(columns=["Sin_Saldo_Inicial", "Saldo_Negativo"], errors="ignore")
    display = display.rename(columns=cols_rename)

    def highlight_negativo(row):
        idx = display.index.get_loc(row.name)
        if idx < len(saldo_neg) and saldo_neg.iloc[idx]:
            return ["background-color: #ffebee; color: #b71c1c"] * len(row)
        return [""] * len(row)

    st.dataframe(display.style.format(fmt).apply(highlight_negativo, axis=1), use_container_width=True, height=600)
...
def exportar_excel(df):
    ...
    headers_grupo = [
        ("Código", 1, 1), ("COMPROBANTE", 2, 5), ("Tipo de Operación", 6, 6),
        ("ENTRADAS", 7, 9), ("SALIDAS", 10, 12), ("SALDO FINAL", 13, 15),
    ]
    bold  = Font(bold=True)
    ...
    for (titulo, col_ini, col_fin) in headers_grupo:
        ...
        cell.font = bold; cell.alignment = center; cell.border = borde
        if col_ini != col_fin:
    ...
    subheaders = ["Código", "Fecha", "Tipo", "Serie", "Número", "Tipo de Operación",
                  "Cantidad", "Costo Unitario", "Costo Total",
                  "Cantidad", "Costo Unitario", "Costo Total",
                  "Cantidad", "Costo Unitario", "Costo Total"]
    for col, sh in enumerate(subheaders, start=1):
        if col in (1, 6): continue
    ...
    for row_idx, row in enumerate(df.itertuples(index=False), start=3):
        ...
        datos = [
            ...
            (row.Tipo, "00"), (str(row.Serie), "@"),
            (numero_val, r'[$-408]00000000'), (row.Tipo_Operacion, "@"),
            (row.Ent_Cantidad, "#,##0.000"), (row.Ent_Costo_Unit, "#,##0.0000"), (row.Ent_Costo_Total, "#,##0.000"),
            (row.Sal_Cantidad, "#,##0.000"), (row.Sal_Costo_Unit, "#,##0.0000"), (row.Sal_Costo_Total, "#,##0.000"),
            (row.Saldo_Cantidad, "#,##0.000"), (row.Saldo_Costo_Unit, "#,##0.0000"), (row.Saldo_Costo_Total, "#,##0.000"),
        ]
        for col, (val, fmt_num) in enumerate(datos, start=1):
            cell = ws.cell(row=row_idx, column=col)
            cell.value = val; cell.number_format = fmt_num; cell.border = borde
            cell.alignment = Alignment(horizontal="center" if col in cols_centradas else "general", vertical="center")
...
with st.sidebar:
    ...
    saldo_file = st.file_uploader("Archivo de saldos iniciales (.xlsx)", type=["xlsx"], key="saldo_ini")
    st.markdown("---")
    st.markdown("**2. Movimientos (Kardex)**")
    uploaded_files = st.file_uploader("Uno o más archivos de movimientos (.xlsx)", type=["xlsx"], accept_multiple_files=True, key="movimientos")
    if saldo_file:
...
if saldo_file:
    ...
    if err_saldo:
        ...
        st.markdown(f'<div class="ok-box">✅ Saldos iniciales cargados: <strong>{len(saldos_iniciales)}</strong> producto(s).</div>', unsafe_allow_html=True)
...
# ── CRÍTICO: Detectar códigos duplicados entre archivos ───────────────────────
duplicados = detectar_duplicados(frames)
if duplicados:
    msgs = [f"<strong>{cod}</strong> aparece en: {', '.join(archs)}" for cod, archs in duplicados.items()]
    st.markdown(
        f'<div class="error-box">❌ <strong>Códigos duplicados en múltiples archivos.</strong> '
        f'Esto causará cálculos incorrectos. Sube un solo archivo por producto:<br>{"<br>".join(msgs)}</div>',
        unsafe_allow_html=True
    )
    st.stop()

# ── Calcular con indicador de progreso ────────────────────────────────────────
with st.spinner("⏳ Calculando saldos finales..."):
...
if sin_saldo:
    ...
    st.markdown(f'<div class="alert-box">⚠️ Productos sin saldo inicial (calculados desde cero): {codigos_str}</div>', unsafe_allow_html=True)

# ── CRÍTICO: Alertar saldos negativos ────────────────────────────────────────
negativos = df_all[df_all["Saldo_Negativo"] == True]
if len(negativos) > 0:
    codigos_neg = negativos["Codigo"].unique().tolist()
    codigos_str = ", ".join([f"<strong>{c}</strong>" for c in codigos_neg])
    st.markdown(
        f'<div class="error-box">❌ <strong>Saldo negativo detectado</strong> en {len(negativos)} fila(s) — '
        f'Productos: {codigos_str}<br>'
        f'Hay más salidas que stock disponible. Las filas afectadas se muestran en rojo en la tabla.</div>',
        unsafe_allow_html=True
    )
...
with col_id1:
    id_buscado = st.text_input("Ingresa el código del producto", placeholder="Ej: 011039")
with col_id2:
...
if id_buscado.strip():
    ...
    if coincidencias:
        ...
        st.warning(f"⚠️ No se encontró el código '{id_buscado.strip()}'.")
...
tipo_filtro = st.radio("Modo de filtro", ["Por Año / Mes", "Por fecha exacta", "Por rango de fechas"], horizontal=True)
...
if tipo_filtro == "Por Año / Mes":
    ...
    with col_m:
        if año_sel == "Todos":
            ...
            meses_en_año = sorted(fechas_validas[fechas_validas.dt.year == año_sel].dt.month.unique().tolist())
    ...
    if año_sel != "Todos":
        if mes_sel == "Todos":
            ...
            df_filtrado = df_filtrado[(df_filtrado["Fecha"].isna()) | ((df_filtrado["Fecha"].dt.year == año_sel) & (df_filtrado["Fecha"].dt.month == mes_sel))]
    ...
    df_filtrado = df_filtrado[(df_filtrado["Fecha"].isna()) | (df_filtrado["Fecha"].dt.date == fecha_exacta)]
    ...
    if fecha_desde <= fecha_hasta:
        df_filtrado = df_filtrado[(df_filtrado["Fecha"].isna()) | ((df_filtrado["Fecha"].dt.date >= fecha_desde) & (df_filtrado["Fecha"].dt.date <= fecha_hasta))]
    else:
...
# ── Métricas y tabla ──────────────────────────────────────────────────────────
st.markdown(f'<div class="section-title">📋 Movimientos <span style="font-weight:400;font-size:0.85rem;color:#888;">({len(df_filtrado)} registros)</span></div>', unsafe_allow_html=True)
...
opcion_export = st.radio(
    "¿Qué deseas exportar?",
    ["Todo el kardex", "Solo lo que se muestra (filtrado)"],
    horizontal=True
)

df_export = df_filtrado.copy() if opcion_export == "Solo lo que se muestra (filtrado)" else df_all.copy()
df_export = df_export.drop(columns=["Sin_Saldo_Inicial", "Saldo_Negativo"], errors="ignore")

with st.spinner("⏳ Preparando archivo Excel..."):
    buffer = exportar_excel(df_export)

nombre_personalizado = st.text_input("Nombre del archivo (opcional)", placeholder=f"kardex_{datetime.date.today().strftime('%Y%m%d')}")
